...
CAssign[lhs_:"",expr_?(!OptionQ[#]&),opts___?OptionQ]:=
  Module[{optvals},
    optvals /; 
      And[
        (optvals = OptionTest[expr,GetShape[lhs],CAssign,Apply[Sequence,FilterRules[Flatten[{opts}],Options[CAssign]]]])=!=$Failed,
        optvals = CMain[lhs,expr,optvals,FilterRules[Flatten[{opts}],Options[Optimize`Optimize]]];
        True
      ]
  ];
